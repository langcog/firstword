...
quartz()
ggplot(data=cdi, aes(x=cdi_cat, y=prop))+
  geom_histogram(stat="identity") +
...
cat.by.age <- d %>%
  select(Category,agesplit,dataset,age,word_standard, real)

#analysis over entire dataset
cat.by.age %<>%
  filter(!is.na(agesplit), Category != "N/A") %>%
  group_by(dataset, agesplit, real, Category) %>%
...
cdi.data <- as.tbl(intersect(cat.by.age, df_mbcdi))%>%
  filter(basetable_ptr_id != "NA", value == "produces", age != "NA", language == "English", sex != "NA", momed.level != "NA", category != "NA")#this is not the best way to do it
...
Age CDF graphs --- with resampling! - Note, there is an issue here that is making the Wordbank data look super wonky
--------------
...
ggplot(wg.producers, aes(x=age,y=prop)) + geom_smooth()
