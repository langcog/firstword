...
```{r, message=FALSE}
#NOTE: section is under construction because Wordbank has been updated since original code written 
...
items <- get.item.data(common.tables$wordmapping,
                       common.tables$instrumentsmap, common.tables$category)
...
#get word data - but I'm not sure what is going on here...
word.data <- items %>%
  filter(language == "English") %>%
  select(id, item, category_id, gloss, definition) %>%
  rename(word=definition, child.id=id) %>%
  as.data.frame


wg.table <- tbl(wordbank,"instruments_wg")

# Get the child id, and demographics 
admin.data <- admins %>%
  select(data_id,age,language, sex, momed.level, birth.order, momed.order) %>%
  filter(language == "English") %>%
...
#Don't need this anymore
# # demos <- select(common.tables$child,sex,momed_id,birth_order) %>%
# #   as.data.frame %>%# Rename id fields
#  # Join age and demographics together


wordbank.data <- as.tbl(left_join(word.data,admin.data, by="sex"))
# child.data <- as.tbl(left_join(admins,demos, by="sex"))
```

Pull in wordbank data
```{r}
#To be fixed after item mappings
...
#Also check the understood words
wds.understood.bykidword <- wg.vocab.words %>% 
...
#            dataset == "Museum" | dataset == "Psycholinguists" & word != "Mama" &
...
```{r}
freqs <- select(df_mbcdi, cdi_cat, word) %>%
  group_by(cdi_cat) %>%
  summarise(n=n()) %>%
  group_by() %>%
  mutate(prop = n/sum(n), total=sum(n), cdi_cat = factor(cdi_cat,
                           levels = cdi_cat[order(prop,cdi_cat,
                                                   decreasing=TRUE)])) 



quartz()
ggplot(data=freqs, aes(x=cdi_cat, y=prop))+
  geom_histogram(stat="identity") +
...
freqs <- bind_rows(select(d.age, word, age, gender, dataset)) %>%
  filter(word == "No", age != "NA", age != "0", age != "1", age != "3") %>%
  group_by(gender, age) %>%
  summarise(n=n()) %>%
  group_by(gender) %>%
  mutate(prop = n/sum(n))
...
```
---
M+V appendix - some quick stuff

Word histogram
```{r}
df <- read.csv("../data/MVapp.csv") #read in MV app data
#histogram of words in all languages by prop
freqs <- df %>%
  filter(word_standard != "N/A") %>%
...
freqs$word_standard <- factor(freqs$word_standard, 
                                  ...
                                  [order(freqs$prop, word_standard, 
                                  decreasing = TRUE)])))
quartz()
ggplot(data = freqs, 
       aes(x=word_standard, y=prop)) +
  geom_bar(stat="identity", position="dodge") +
  ylab("Proportion as first word") + 
  xlab("Word") +
  scale_fill_brewer(palette="Set1") + 
  theme(axis.text.x=element_text(angle=90, hjust=1, vjust=.5, size=8))
...
freqs <- df %>%
  filter(word_standard != "N/A") %>%
  group_by(language, word_standard) %>%
  summarise(n=n()) %>%
  top_n(5) %>%
  arrange(n, language) %>%
  group_by(language)

#CDI cats by agesplit
freqs <- df %>%
  filter(word_standard != "N/A") %>%
  group_by(cdi_cat, agesplit) %>%
  summarise(n = n()) %>%
  group_by(agesplit,add=FALSE) %>%
...
quartz()
ggplot(data = freqs, 
       aes(x=cdi_cat, y=prop)) +
  geom_bar(stat="identity", position="dodge") +
  ylab("Proportion as first word") + 
  xlab("Word") +
  scale_fill_brewer(palette="Set1") + 
  theme(axis.text.x=element_text(angle=90, hjust=1, vjust=.5, size=8)) + facet_wrap(~agesplit)
